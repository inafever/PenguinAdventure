...
- 주인공 스프라이트는 `assets/main-characters/`의 `right` 프레임을 씁니다. 펭귄 프레임은 모두 188×264라서 자르면 흔들립니다. 선택 창 썸네일은 `walk_right_1` 정지 컷입니다(프레임을 돌리지 않음). 플레이 중 크기는 실제 그림(여백 제외)을 72×84 상자에 contain합니다.
